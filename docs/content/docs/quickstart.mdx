---
title: Quick start - connect Gmail to your MCP client
description: Install DaySurface, connect your Gmail account, and run your first inbox command from Claude, ChatGPT, or the CLI in under a minute.
---

DaySurface gives you three ways to interact with the same set of Gmail tools:

- **CLI** - Run commands in your terminal
- **MCP** - Connect AI assistants like Claude directly
- **REST API** - Call endpoints over HTTP with authentication

Pick the interface that fits your workflow, or use all three.

## Installation

import { Step, Steps } from 'fumadocs-ui/components/steps';

<Steps>
<Step>

### Install from source

```bash
git clone https://github.com/Miyamura80/DaySurface.git
cd DaySurface
uv sync
```

</Step>
<Step>

### Verify the installation

```bash
uv run daysurface --version
uv run daysurface --help
```

</Step>
<Step>

### Try a command

```bash
uv run daysurface greet Alice
# Hello, Alice!
```

</Step>
</Steps>

## Next steps

<Cards>
  <Card title="CLI" href="/docs/cli" />
  <Card title="MCP Server" href="/docs/mcp" />
  <Card title="REST API" href="/docs/api" />
</Cards>
