...
Liveness only: the endpoint answers `200` with a fixed body whenever the
process is serving, and deliberately runs no dependency probes. Readiness -
whether the database, Redis and Stripe are actually reachable - is on
`/health/detail` below, so that a slow or hanging dependency can never stall
the probe a platform healthcheck restarts the container over.
