...
<Callout type="info">
`/health` is the canonical status probe (no auth required). It returns
`{"status": "ok"}` or `{"status": "degraded"}` and nothing else - poll it to
distinguish a server-side incident from a client-side error before retrying.
The per-component breakdown (`api`, `database`, `redis`, `stripe`) is on the
authenticated `/health/detail` - see the
