...
<Callout type="info">
`/health` is the public liveness probe (no auth required). It returns
`{"status": "ok"}` and nothing else, and runs no dependency probes - so it
tells you the process is reachable, not that its backends are healthy. For
readiness, use the authenticated `/health/detail`, which carries the
per-component breakdown (`api`, `database`, `redis`, `stripe`) - see the
[API overview](/docs/api#health-check).
</Callout>
...
```bash
curl https://api.daysurface.com/api/v1/auth/me
```
