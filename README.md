...
| | |
|:--|:--|
| **`gmail_inbox`** - curated inbox with importance scores and labels, beside a thread reader showing a pending draft and AI-drafted replies<br><br><img src="media/mcp-ui/gmail-inbox.png" alt="gmail_inbox MCP App: curated inbox list beside an open thread with a pending draft"> | **`gmail_composer`** - the draft view every compose/reply tool opens, rendered by the inbox app: conversation above, Gmail-style To / Cc/Bcc and body, send / attach / discard<br><br><img src="media/mcp-ui/gmail-composer.png" alt="gmail_composer: Gmail-style draft card under the conversation, with To, Cc/Bcc, body and a Send / attach / discard toolbar"> |
...
Every shot is inside a Claude conversation because that is where these UIs live: the user asks, the model calls a tool, and the app renders as that tool's result, inline in the thread. Nothing here is a standalone web app.

The cards are rendered from the committed app bundles by the fixture host in `mcp_server/dev_preview/` - no server, Gmail account, or OAuth involved. Reproduce one with `make preview_app APP=<name> CHAT=claude`. The composer shot is `APP=gmail_inbox SCENARIO=gmail_composer`, since that view is served by the inbox app rather than by a bundle of its own.
