...
Screenshots are rendered from the committed app bundles by the fixture host in `mcp_server/dev_preview/` - no server, Gmail account, or OAuth involved. Reproduce them with `make preview_app APP=<name>`; the composer shot is `APP=gmail_inbox` with the drafted thread opened and its draft card clicked, since that is the path a compose or reply tool puts a user on.
