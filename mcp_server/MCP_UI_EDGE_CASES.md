...
| A13 | Concurrent writes to one draft (rapid attachment adds, autosave overlapping an upload) | **Known gap.** `InlineComposer` serializes nothing: `persistDraft` fires per debounce with no write queue, so two `save_draft` calls can read the same `existingAttachments` snapshot and the later reply wins, silently dropping a file. The removed orphan solved this with a `saveChainRef` promise chain; recover it from `fda8dea:mcp_server/apps/gmail_composer/src/Composer.tsx`. Related and also absent from `useComposerAttachments.ts`: a cumulative 25 MB size preflight that fails closed on unknown size, and zero-byte file rejection. |
