...
| A7 | Host doesn't implement `ui/update-model-context` | Where the push exists it is best-effort try/catch: on rejection the Sent/Discarded UI state is unaffected and no error is shown - the model just stays uninformed, which is the pre-push status quo. **Known gap: nothing in the tree implements this today.** `InlineComposer` inside `apps/gmail_inbox/` never calls `updateModelContext`, so after a user-initiated send or discard the model still believes the draft is open. The only implementation lived in the orphaned `apps/gmail_composer/`, removed in the commit that closed A12; recover it from `fda8dea:mcp_server/apps/gmail_composer/src/modelContext.ts` when closing this. |
...
| A12 | An app directory exists but no enhancer ever sends its `ui://` URI | `_register_app_resources` walks `mcp_server/apps/` and registers a resource per directory, so an unreferenced app is still advertised to hosts and still force-included in the wheel. Note the registration is directory-driven, not build-driven: a directory with no `dist/mcp-app.html` still gets a resource that serves the A1 stub, so removal means deleting the directory, not just its bundle. `apps/gmail_composer/` was in this state from the initial commit until it was removed - every composer tool has always sent `INBOX_URI`. Treat divergence between an orphaned app and its live replacement as a source of silent regressions: the orphan held working implementations of guards the shipping composer lacks (see A7 and A13), and because nothing rendered it, its 24 tests never guarded anything. When an app is superseded, delete it and port what is missing in the same change - leaving it as a reference copy is how the gap goes unnoticed. |
| A13 | Concurrent writes to one draft (rapid attachment adds, autosave overlapping an upload) | **Known gap.** `InlineComposer` serializes nothing: `persistDraft` fires per debounce with no write queue, so two `save_draft` calls can read the same `existingAttachments` snapshot and the later reply wins, silently dropping a file. The removed orphan solved this with a `saveChainRef` promise chain; recover it from `fda8dea:mcp_server/apps/gmail_composer/src/Composer.tsx`. The related attachment preflight (cumulative 25 MB failing closed on unknown size, zero-byte rejection, and bytes claimed while a read is still in flight) **is** implemented in `useComposerAttachments.ts`; serialization is what remains missing. |
