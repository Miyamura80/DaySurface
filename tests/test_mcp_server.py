...
class TestMCPServer(TestTemplate):
    ...
    def test_mutating_webhook_tools_hidden_from_llm_but_still_registered(self):
        ...
        # register/rotate/remove a webhook (an exfil primitive). These are off
        # the LLM tool surface (users drive them via the Settings app) but stay
        # in the registry so the HTTP transport and app-only tools keep them.
        tools = mcp._tool_manager._tools
        service_names = {e.name for e in get_registry()}
        for name in (
            "webhook_subscribe",
            "webhook_unsubscribe",
        ...
        # Read-only listing stays available to the model.
